...
with tab1: 
    ...
    Pricing , Trend , Rupturas    = st.tabs([ "Pricing" ,"Trend","Rupturas" ])
        ...
        with st.expander('Filtros', expanded= True):
            ...
            with col[1]:
                ...
                def df_modelos(): 
                    ...
                    df_modelo = pipe_df_modelos(df_order_previsao, df_ofertao_inicial, df_trafego_previsao , df_produtos_previsao , df_users_previsao,  query_concorrencia , regional_modelo, size_modelo , [categorias_forecasting], 'ean', '2022-01-01', '2024-05-01' )
...
# %% Feature Modelo 
        ...
        with st.expander('Filtros', expanded= True):
            ...
            with st.form(key = "features_modelo"):
                ...
                with col[1]:
                    ...
                    with col3:
                        ...
                        columns_ofertao_modelo = st.multiselect("Ofertão Columns", columns_ofertao  , default = df_modelo[columns_ofertao].filter(regex=sku_forecasting[0]).columns.to_list() )           
            ...
            var_forecasting = df_modelo[[metricas_forecasting]].columns.to_list()
            var_forecasting = [i for i in var_forecasting if i.find( sku_forecasting[0] ,0)>=0 ]


            df_modelo = df_modelo[ columns_gmv_modelo + columns_positivacao + columns_ofertao_modelo + columns_preco_modelo  + columns_preco_concorrente]
            df_modelo
...
# %% Pipeline Modelo
        ...
        with st.expander('Filtros', expanded= True):
            ...
            with st.form(key = "pipeline_modelo"):
                ...
                lag_features =   st.multiselect('Lag/Wind Features',  columns_gmv_modelo + columns_positivacao , default = columns_gmv_modelo )
              ...
              #  df_modelo = df_modelo[columns_gmv_modelo + columns_positivacao + columns_ofertao_modelo + columns_preco_modelo  + columns_preco_concorrente]
                ...
                def pipe(df_modelo_final):
                    ...
                    lagf = LagFeatures(variables = pipe_colums + ofertao_final ,freq = day_lag  ,missing_values = "ignore",) 
                    winf = WindowFeatures(variables = pipe_colums ,  window =  day_wind ,freq = "1D", missing_values = "ignore",) 
        ...
        # pipe_colums = gmv_columns + positivacao_columns   


    
        # df_modelo_pipe =   df_modelo 



        # if sku != 'categoria': 
            
        #     pipe_colums.remove('Gmv Categoria_' + categoria)
        #     pipe_colums.remove('Positivação Categoria_' + categoria)
        #     pipe_colums = [i for i in pipe_colums if i.find( sku ,0)>=0 ] 
        #     precos = [i for i in precos if i.find(sku ,0)>=0 ] 
        #     precos_concorrente = [i for i in precos_concorrente if i.find( sku ,0)>=0 ]
        #     col_predicao = [i for i in listas['columns_gmv'] if i.find(sku ,0)>=0 ] 
        # else:
        #     col_predicao = 'Gmv Categoria_' + categoria

 
        # metricas_calculadas = []
        # st.text('posit')

        # # % POSITIVAÇÂO E CONVERSÃO           
